...
export function SecretNotesClient(props: SecretNotesClientProps) {
  ...
  const [selectedId, setSelectedId] = useState<number>(props.notes[0]?.id ?? 0);
  const { completedNoteIds, setCompleted } = useCompletionStorage();
  ...
  const selectedNote = useMemo(() => {
    return filteredNotes.find((note) => note.id === selectedId) ?? filteredNotes[0] ?? null;
  }, [filteredNotes, selectedId]);

  const handleToggleCompleted = (id: number, checked: boolean) => {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          ...
          {filteredNotes.length > 0 ? (
            filteredNotes.map((note) => (
              <NoteCard
                ...
                isCompleted={completedNoteIds.has(note.id)}
      ...
      <NoteDetailPanel
        note={selectedNote}
        isCompleted={selectedNote ? completedNoteIds.has(selectedNote.id) : false}
      />
    </div>
  );
}
